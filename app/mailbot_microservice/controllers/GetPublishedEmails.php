...
class GetPublishedEmails extends Controller
{
    ...
    private function retrieveFromDatabase($conn, String $email, $noPage, $noSections) {
        ...
        $sql = "SELECT count(*) FROM (SELECT ROWNUM rn, t.* FROM (SELECT * FROM usermails WHERE user_email = :user_email AND published = 1 ORDER BY content_email DESC) t) WHERE rn < :searchTill AND rn > :searchFrom";
        ...
        $user->setEmail($email);
        $sql = "SELECT * FROM (SELECT ROWNUM rn, t.* FROM (SELECT * FROM usermails WHERE user_email = :user_email AND published = 1 ORDER BY content_email DESC) t) WHERE rn < :searchTill AND rn > :searchFrom";
        $stid = oci_parse($conn, $sql);
        oci_bind_by_name($stid, ':user_email', $email);
