...
class GetArchivedEmails extends Controller
{
    ...
    private function retrieveFromDatabase($conn, String $email, $noPage, $noSections) {
        ...
        $sql = "SELECT count(*) FROM (SELECT ROWNUM rn, t.* FROM (SELECT * FROM friendsmails WHERE user_email = :user_email ORDER BY content_email DESC) t) WHERE rn < :searchTill AND rn > :searchFrom";
