...
const requestDataTime = async()=>{
        const post_id = document.getElementById('keytag').getAttribute('sneeze');
        console.log(post_id);
        const timescale = document.getElementById('viewsdatascope').value;
        var url = "http://127.0.0.1:9090/ords/mailbot/get_stats/";
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4) {  
            ...
            if(timescale==1){
              ...
              for(var i = 0; i < rawtimedata.length; i++){
                ...
                for(var i = 0; i < json.x_ret.length; i++){
                    ...
                    console.log(date.toLocaleString()+" "+new Date(Date.parse(new String(json.x_ret[i]).split('#')[0])).toLocaleString());
            ...
            var html = '<html><head><title>Views over time</title></head><body><div style="width:fit-content; margin:auto;"><h1>Views over time</h1><table><tr><th>Time</th><th>Views</th></tr>';
            for(var i = 0; i < rawtimedata2.length; i++){
      ...
      const requestDataCountries = async() => {
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4) {  
            ...
            //write to html file
            var html = '<html><head><title>Countries of visitors</title></head><body><div style="width:fit-content; margin:auto;"><h1>Countries of visitors</h1><table><tr><th>Country</th><th>Views</th></tr>';
            ...
            for(var i = 0; i < x.length; i++){
              xml += '<country><code>'+x[i]+'</code><views>'+y[i]+'</views></country>';
            }
            xml += '</countries></xml>';
    ...
    const getLastView = async() => {
      const post_id = document.getElementById('keytag').getAttribute('sneeze');
      var url = "http://127.0.0.1:9090/ords/mailbot/get_last_viewed/";
      var xhr = new XMLHttpRequest();
      xhr.open("POST", url);
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
          const json = JSON.parse(xhr.responseText);
          const lastView = json.last_date;
          document.getElementById('last_view').innerHTML = lastView;
        }
      }
