...
class DisplayArchivedEmailsOnly extends Controller
{
    public function index ($noPage = 1, $noSections = 6) {
        ...
        if($jwt !== '') {
            ...
            if($noPage == 0)
               $noPage = 1;
            $emailFriend = $_POST['emailSender'];
            $request_as_string = "http://localhost:8181/public/GetArchivedEmailsOnly/index?email=".$email."&emailFriend=". $emailFriend . "&noPage=". $noPage.'&noSections='.$noSections;
            $c = curl_init ();
